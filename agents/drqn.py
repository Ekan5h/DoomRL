...
imgno = 0
def preprocessImg(img, size, game):
    global imgno
    img = np.rollaxis(img, 0, 3)    # It becomes (640, 480, 3)
    if game % 1000 == 0:
        skimage.io.imsave("gif/{}.png".format(imgno), img)
        imgno += 1
...
def trainDRQN(game, agent):
    ...
    s_t = preprocessImg(s_t, size=(img_rows, img_cols), 1)
    ...
    while not game.is_episode_finished():
        ...
        s_t1 = game_state.screen_buffer
        misc = game_state.game_variables
        s_t1 = preprocessImg(s_t1, size=(img_rows, img_cols), GAME+1)
